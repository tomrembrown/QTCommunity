...
var urls = ['http://www.the519.org/calendar/?type=daily#', 'http://www.the519.org/calendar/?type=upcoming#']
...
for (var i in urls) {
  ...
  qtFetch.getURL(urls[i]).then(function (content) {
    content = content.data

    var document = new dom().parseFromString(content)
    var page_urls = xpath.select('//div[contains(@itemtype, "http://schema.org/Event")]/div[contains(@class, "event")]/a/@href', document) // https://johnresig.com/blog/xpath-css-selectors/

    var output = []

    for (var j in page_urls) {
      var page_url = page_urls[j].toString().replace('\"', '').replace('\"', '').replace('href=', '').trim()

      if (output.indexOf(page_url) === -1) {
        output.push(page_url)

        console.log('\tProcessing ' + page_url + ' : ' + md5(page_url) + ' \n')
        qtFetch.getURL(page_url).then(function (page_content) { // page_content is in array
          ...
          var description = xpath.select('//*[contains(@class, "program-description")]/*', page_document)
          ...
          for (var k in data['startDate']) {
            ...
            var timestamp_text = xpath.select1('string(//div[contains(@content, "' + timestamp + '")])', page_document).toString().trim().replace(regex3, '')
