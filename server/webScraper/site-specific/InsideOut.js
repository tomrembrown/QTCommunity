...
qtFetch.getURL(url).then(
  function (content) {
    ...
    var page_urls = xpath.select(
      '//div[contains(@class, "block event")]/a/@href',
      document
    )
    ...
    for (var i in page_urls) {
      ...
      qtFetch.getURL(page_url).then(
        function (page_content) {
          ...
          var date = xpath.select(
            'string(//div[contains(@class, "detail")]/div[contains(@class, "meta")]/div[contains(@class, "date")])',
            page_document
          )
          var time = xpath.select(
            'string(//div[contains(@class, "detail")]/div[contains(@class, "meta")]/div[contains(@class, "time")])',
            page_document
          )
          var website = xpath.select(
            'string(//div[contains(@class, "detail")]/div[contains(@class, "meta")]/div[contains(@class, "place")]//a/@href)',
            page_document
          )

          var place = xpath.select(
            'string(//div[contains(@class, "detail")]/div[contains(@class, "meta")]/div[contains(@class, "place")])',
            page_document
          )
          place = place.replace(regex3, '')

          var description = xpath.select(
            '//div[contains(@class, "detail")]/div[contains(@class, "description")]/*',
            page_document
          )
          ...
          var image = xpath.select(
            'string(//div[contains(@class, "contain")]/img/@src)',
            page_document
          )
