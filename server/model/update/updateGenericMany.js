...
const updateGenericMany = async function(columnList, rowsArray, tableName) {
  ...
  try {
    ...
    let columnTypeQuery = "SELECT data_type " +
                          "FROM INFORMATION_SCHEMA.COLUMNS " +
                          "WHERE table_name = '" + tableName +
                          "' AND column_name IN ('" + columnList.join("', '") + "');"
    ...
    for (let col = 1; col < numCols; col++) {
      ...
      updateTableQuery += columnList[col] + ' = m.' + columnList[col] + '::' + dataTypes[col].data_type
    }
    ...
    updateTableQuery += ') WHERE m. ' + columnList[0] + '::' + dataTypes[0].data_type + ' = t.' + columnList[0] +';'
